...
function Paginator({selectPageEvent,pageNumber,pageCount,disabled}) {
  ...
  return (
    <Pagination className={(disabled)?'disabled-div':''}>
      <Pagination.Prev onClick={() => selectPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1 || pageCount === 0} />
      {
        (pageCount > 0)?
        renderPaginationItems()
        :
        <Pagination.Item key={1} active disabled>{0}</Pagination.Item>
      }
      <Pagination.Next onClick={() => selectPage(prev => Math.min(prev + 1, pageCount))} disabled={currentPage === pageCount || pageCount === 0} />
    </Pagination>
  );
}
