...
class TestPage:
    def test_page_should_fail_when_constructed_with_negative_number(self):
        with raises(InvalidPageNumberError) as error:
            Page(-1, 2)
        assert "-1 is not a valid page number" in str(error)

    def test_page_should_fail_when_constructed_with_negative_size(self):
    ...
    def test_page_sets_default_number_and_size_when_not_given(self):
        page = Page()
